...
export class UsersRouter extends ClassesRouter {
  ...
  handleMe (req) {
    if (!req.info || !req.info.sessionToken) {
      throw new Parse.Error(Parse.Error.INVALID_SESSION_TOKEN, `invalid session token || req was: ${JSON.stringify(req)}`)
    }
    ...
    return rest.find(req.config, Auth.master(req.config), '_Session',
      ...
      .then((response) => {
        if (!response.results ||
          ...
          throw new Parse.Error(Parse.Error.INVALID_SESSION_TOKEN, `invalid session token in users router: ${sessionToken} || response was: ${JSON.stringify(response)}`)
        } else {
